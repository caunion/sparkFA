...
class PCAUtils {
	...
	*/
	...
	/**
	 * Randomly initialized psi, DMR
	 */

	static Matrix randPsi(int rows){
		double[] diagVals = new double[rows];
		for(int i = 0; i < diagVals.length; i++) {
			diagVals[i] = random.nextDouble();
		}
		Matrix psiInit = new DiagonalMatrix(diagVals);
		return psiInit;
...
/*	static Matrix invDiag(Matrix m){

        return m.viewDiagonal().assign(m.viewDiagonal().assign(Functions.INV));
        //how do I properly set the diagonals to be this?
	...
	/*static double [] MatrixTimeDenseVector(Matrix matrix, Vector vector, double[] arr){
        int nRows = matrix.numRows();
        int nCols = matrix.numCols();
        for (int c=0; c< nCols; c++){
	...
	public static void outerProductYiYMean(org.apache.spark.mllib.linalg.Vector yi, org.apache.mahout.math.Vector ym,
											   double[][] resArray, int[] nonZeroIndices) {
		// 1. Sum(Yi' x Ym)
		...
		for(i=0; i < nonZeroIndices.length; i++)
		{
			...
			//for (int xCol = 0; xCol < xSize; xCol++) {
			//int xCol=0;
			Iterator<Vector.Element> it = ym.nonZeroes().iterator();
			while(it.hasNext()){
				xCol = it.next().index();
				resArray[yRow][xCol] +=  ym.get(xCol) * yScale;
				//xCol++;
			}
	...
	public static void outerProductYMeanYi(org.apache.mahout.math.Vector ym, org.apache.spark.mllib.linalg.Vector yi,
										   double[][] resArray, int[] nonZeroIndices) {
		// 1. Sum(Ym' x Yi)
		...
		while(it.hasNext()){
			xRow = it.next().index();
			for (int i = 0; i < nonZeroIndices.length; i++) {
				yCol=nonZeroIndices[i];
				yScale = yi.apply(yCol);
				//double centeredValue = xi[xCol] - xm.getQuick(xCol);
				resArray[xRow][yCol] += yScale * ym.get(xRow);
			}
		}
	}
	...
	public static void outerProductYi(org.apache.spark.mllib.linalg.Vector idxVect,
									  double [][] resArray, int [] nonZeroIndices){
		double yScaleRow=0;
		double yScaleCol=0;
		int yRow, yCol, jRev, yRowMult;
		for(int i=0; i<nonZeroIndices.length; i++){
			yRow=nonZeroIndices[i];
			yScaleRow=idxVect.apply(yRow);

			for (int j = 0; j<nonZeroIndices.length; j++) {
				//jRev = nonZeroIndices.length - 1 - j;
				yRowMult=nonZeroIndices[j];
				yScaleCol=idxVect.apply(yRowMult);
				yCol=nonZeroIndices[j];
				resArray[yRow][yCol] += yScaleRow * yScaleCol;
				int cat = 3;
			}
		}
	...
	public static void outerProductYmean(org.apache.mahout.math.Vector ym, double[][] resArray) {
		//double [] xi = yi;
		int ymLength = ym.size();
		Iterator<Vector.Element> it = ym.nonZeroes().iterator();
		...
		while(it.hasNext()){
			yRow = it.next().index();
			for (int xCol = 0; xCol < ymLength; xCol++) {
				yCol = ym.size() - 1 - xCol;
				//double centeredValue = xi[xCol] - xm.getQuick(xCol);
				resArray[yRow][xCol] += ym.get(yCol) * ym.get(xCol);
			}
		}
	}
